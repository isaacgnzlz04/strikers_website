...
const PRESETS = {
  top: { position: 'top', height: '2rem' },
  bottom: { position: 'bottom', height: '2rem' },
  left: { position: 'left', height: '2rem' },
  right: { position: 'right', height: '2rem' },
  subtle: { height: '4rem', strength: 1, opacity: 0.8, divCount: 3 },
  intense: { height: '10rem', strength: 4, divCount: 8, exponential: true },
  smooth: { height: '8rem', curve: 'bezier', divCount: 10 },
  sharp: { height: '5rem', curve: 'linear', divCount: 4 },
  header: { position: 'top', height: '8rem', curve: 'ease-out' },
  footer: { position: 'bottom', height: '2rem', curve: 'ease-out' },
  sidebar: { position: 'left', height: '6rem', strength: 2.5 },
  'page-header': { position: 'top', height: '10rem', target: 'page', strength: 3 },
  'page-footer': { position: 'bottom', height: '4rem', target: 'page', strength: 3 }
};
...
const GradualBlur = props => {
  ...
  const blurDivs = useMemo(() => {
    // Skip rendering blur divs on mobile for performance
    if (isMobile) {
      return [];
    }
  ...
  const containerStyle = useMemo(() => {
    ...
    const baseStyle = {
      ...
      opacity: config.animated ? (isVisible ? 1 : 0) : 1,
